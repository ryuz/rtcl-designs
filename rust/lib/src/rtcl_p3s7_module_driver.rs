...
/// PMOD mode register (drives `out_pmod_mode`, 16-bit)
const REGADR_PMOD_MODE: u16 = 0x0b0;
/// PMOD GPIO input register (reads `in_pmod_data`, 8-bit)
const REGADR_PMOD_GPIO_IN: u16 = 0x00b2;
/// PMOD GPIO output data register (drives `out_pmod_data`, 8-bit)
const REGADR_PMOD_GPIO_OUT: u16 = 0x00b3;
...
impl<E: core::fmt::Display> core::fmt::Display for RtclP3s7ModuleDriverError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            RtclP3s7ModuleDriverError::UnsupportedDphySpeed => {
                write!(f, "Unsupported D-PHY speed setting")
            }
            RtclP3s7ModuleDriverError::ReceiverCalibrationFailed => {
                write!(f, "Receiver calibration failed")
            }
            RtclP3s7ModuleDriverError::SpiRomOperationTimeout => {
                write!(f, "SPI ROM operation timeout")
            }
            RtclP3s7ModuleDriverError::SensorPowerGoodFailed => {
                write!(f, "Sensor power good signal indicates failure")
            }
        }
    }
}
...
impl<I2C: I2cHal> RtclP3s7ModuleDriver<I2C> {
    ...
    pub fn softeare_reset(&mut self) -> Result<(), RtclP3s7ModuleDriverError<I2C::Error>> {
        // ソフトウェアリセット発行
    ...
    pub fn sensor_ready(&mut self) -> Result<bool, RtclP3s7ModuleDriverError<I2C::Error>> {
        Ok(self.read_i2c(REG_P3S7_SENSOR_READY)? != 0)
    }

    pub fn sensor_pgood(&mut self) -> Result<bool, RtclP3s7ModuleDriverError<I2C::Error>> {
    ...
    pub fn spi_rom_command_write(
        ...
        while index < data.len() {
            if index + 1 < data.len() {
                let addr = if index + 2 >= data.len() && last {
                    0x5003
                } else {
                    0x5002
                };
                ...
                let addr = if index + 1 >= data.len() && last {
                    0x5001
                } else {
                    0x5000
                };
    ...
    pub fn spi_rom_id(&mut self) -> Result<[u8; 3], RtclP3s7ModuleDriverError<I2C::Error>> {
        let cmd: [u8; 1] = [0x9f];
    ...
    pub fn spi_rom_read_status_register(
        &mut self,
    ) -> Result<u8, RtclP3s7ModuleDriverError<I2C::Error>> {
    ...
    /// # Arguments
    ///
    /// * `mode` - Value to write to the PMOD mode control register
    ///
    /// # Errors
    ///
    /// Returns an error if I2C communication fails
    pub fn set_pmod_mode(
        &mut self,
        mode: u16,
    ) -> Result<(), RtclP3s7ModuleDriverError<I2C::Error>> {
    ...
    /// # Returns
    ///
    /// Current PMOD GPIO input value (`REGADR_PMOD_GPIO_IN`, lower 8 bits)
    ///
    /// # Errors
    ///
    /// Returns an error if I2C communication fails
    pub fn read_pmod(
        ...
        Ok(self.read_i2c(REGADR_PMOD_GPIO_IN)? as u8)
    }

    /// Set PMOD GPIO output data
    ///
    /// # Arguments
    ///
    /// * `value` - 8-bit output value to drive to PMOD GPIO pins
    ///
    /// # Errors
    ///
    /// Returns an error if I2C communication fails
    pub fn set_gpio_out(
        ...
        self.write_i2c(REGADR_PMOD_GPIO_OUT, value as u16)?;
        Ok(())
    }

    /// Set PMOD GPIO pin direction
    ///
    /// # Arguments
    ///
    /// * `dir` - Direction bit mask (`1`: output, `0`: input)
    ///
    /// # Errors
    ///
    /// Returns an error if I2C communication fails
    pub fn set_gpio_dir(
        ...
        self.write_i2c(REGADR_PMOD_GPIO_DIR, dir as u16)?;
        Ok(())
    }
    ...
    pub fn set_sensor_receiver_enable(
        ...
        if enable {
            ...
            if cam_calib_status != 0x01 {
                if self.read_i2c(REG_P3S7_SENSOR_PGOOD_EN)? != 0
                    && self.read_i2c(REG_P3S7_SENSOR_PGOOD)? == 0
                {
    ...
    ///
    /// Returns an error if I2C communication fails
    pub fn set_digital_gain_linear(
        &mut self,
        linear_gain: f32,
    ) -> Result<(), RtclP3s7ModuleDriverError<I2C::Error>> {
    ...
    pub fn write_read_i2c(
        &mut self,
        addr: u16,
        data: u16,
    ) -> Result<u16, RtclP3s7ModuleDriverError<I2C::Error>> {
        let addr = addr << 1;
        let wbuf: [u8; 4] = [
            ((addr >> 8) & 0xff) as u8,
            ((addr >> 0) & 0xff) as u8,
    ...
    pub fn module_reg_dump(&mut self) -> Result<(), RtclP3s7ModuleDriverError<I2C::Error>> {
        println!(
            "REG_P3S7_MODULE_ID        : 0x{:04x}",
            self.read_i2c(REG_P3S7_MODULE_ID)?
        );
        println!(
            "REG_P3S7_MODULE_VERSION   : 0x{:04x}",
            self.read_i2c(REG_P3S7_MODULE_VERSION)?
        );
        println!(
            "REG_P3S7_SENSOR_ENABLE    : 0x{:04x}",
            self.read_i2c(REG_P3S7_SENSOR_ENABLE)?
        );
        println!(
            "REG_P3S7_SENSOR_READY     : 0x{:04x}",
            self.read_i2c(REG_P3S7_SENSOR_READY)?
        );
        println!(
            "REG_P3S7_RECEIVER_RESET   : 0x{:04x}",
            self.read_i2c(REG_P3S7_RECEIVER_RESET)?
        );
        println!(
            "REG_P3S7_RECEIVER_CLK_DLY : 0x{:04x}",
            self.read_i2c(REG_P3S7_RECEIVER_CLK_DLY)?
        );
        println!(
            "REG_P3S7_ALIGN_RESET      : 0x{:04x}",
            self.read_i2c(REG_P3S7_ALIGN_RESET)?
        );
        println!(
            "REG_P3S7_ALIGN_PATTERN    : 0x{:04x}",
            self.read_i2c(REG_P3S7_ALIGN_PATTERN)?
        );
        println!(
            "REG_P3S7_ALIGN_STATUS     : 0x{:04x}",
            self.read_i2c(REG_P3S7_ALIGN_STATUS)?
        );
        println!(
            "REG_P3S7_CLIP_ENABLE      : 0x{:04x}",
            self.read_i2c(REG_P3S7_CLIP_ENABLE)?
        );
        println!(
            "REG_P3S7_CSI_MODE         : 0x{:04x}",
            self.read_i2c(REG_P3S7_CSI_MODE)?
        );
        println!(
            "REG_P3S7_CSI_DT           : 0x{:04x}",
            self.read_i2c(REG_P3S7_CSI_DT)?
        );
        println!(
            "REG_P3S7_CSI_WC           : 0x{:04x}",
            self.read_i2c(REG_P3S7_CSI_WC)?
        );
        println!(
            "REG_P3S7_DPHY_CORE_RESET  : 0x{:04x}",
            self.read_i2c(REG_P3S7_DPHY_CORE_RESET)?
        );
        println!(
            "REG_P3S7_DPHY_SYS_RESET   : 0x{:04x}",
            self.read_i2c(REG_P3S7_DPHY_SYS_RESET)?
        );
        println!(
            "REG_P3S7_DPHY_INIT_DONE   : 0x{:04x}",
            self.read_i2c(REG_P3S7_DPHY_INIT_DONE)?
        );
        println!(
            "REG_P3S7_MMCM_CONTROL     : 0x{:04x}",
            self.read_i2c(REG_P3S7_MMCM_CONTROL)?
        );
        println!(
            "REG_P3S7_PLL_CONTROL      : 0x{:04x}",
            self.read_i2c(REG_P3S7_PLL_CONTROL)?
        );
        Ok(())
    }
}
