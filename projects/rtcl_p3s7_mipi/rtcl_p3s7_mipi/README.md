# PYTHON300 + Spartan7 センサー FPGAデザイン

## 概要

[グローバルシャッターMIPI高速度カメラ](https://rtc-lab.com/products/rtcl-cam-p3s7-mipi/)(設計は[こちら](https://github.com/ryuz/rtcl-p3s7-mipi))の Spartan-7 用のFPGAデザインです。

MIPI-CSI規格を利用せずに D-PHY 上に独自プロトコルで高速伝送することが可能です。
その際には画像1フレームを1パケットとして転送することで伝送帯域を有効に使い、高速度撮影を可能としています。

PYTHON300のデータシートはオンセミ社より[こちら](https://www.onsemi.jp/products/sensors/image-sensors/python300)で公開されているので、随時参照してください。

まだ試作段階で、今後まだ大きな変更が入る可能性がある点、ご了承ください。


## 環境

FPGAは Linux 版の Vivado 2024.2 で開発しておりますが、おそらく新しいバージョンでも大丈夫です。

JTAGダウンロードケーブルは、Digilent社の[JTAG-HS3](https://digilent.com/shop/jtag-hs3-programming-cable/)で確認していますが、おそらく他のものでも大丈夫です。


## プロジェクトのビルド方法

### bitstream の作成

Vivado が使えるように

```bash
source /tools/Xilinx/Vivado/2024.2/settings64.sh
```

したのちに

```bash
cd projects/rtcl_p3s7/rtcl_p3s7_mipi/syn/tcl
make
```

とすると

`projects/rtcl_p3s7/rtcl_p3s7_mipi/syn/tcl/`

もしくは

`projects/rtcl_p3s7/rtcl_p3s7_mipi/syn/vivado2024.2/rtcl_p3s7_mipi.xpr` に GUI 用のプロジェクトがあるので、Vivado の GUI から開いてご利用ください。

GUI 版ではさらに下記で mcs ファイルを作成でき、

```tcl
write_cfgmem -format mcs -size 2 -interface spix4 -loadbit "up 0x0 rtcl_p3s7_mipi.runs/impl_1/rtcl_p3s7_mipi.bit" -file rtcl_p3s7_mipi.runs/impl_1/rtcl_p3s7_mipi.mcs -force
```

下記でバイナリファイルが作成できます。

```tcl
write_cfgmem -format bin -size 2 -interface spix4 -loadbit "up 0x0 rtcl_p3s7_mipi.runs/impl_1/rtcl_p3s7_mipi.bit" -file rtcl_p3s7_mipi.runs/impl_1/rtcl_p3s7_mipi.bin -force
```

### コンフィギュレーション ROM への書き込み

JTAGダウンロードケーブルを保有しており、単一イメージを書き換えて使うというスタンダードな方法と、
ゴールデンイメージとアップデートイメージの２つをROMに配置しておき、MIPIケーブルのI2C経由で
アップデートイメージを書き換えて使う方法の２つがあります。

#### 単一イメージを書き込む場合

GUI 版では、bitファイル作成後にコンフィギュレーションROM用の mcs ファイルを作成するために、tcl から


と実行するか、GUI のツールから mcs を生成する必要がりますのでご注意ください。

mcs ファイルは、SPIフラッシュに書き込む際に必要です。

is25lp016d-spi-x1_x2_x4

を選択して書き込んでください。

その際、Stage of non-config mem I/O pins は pull-up を選んで下さい。

#### アップデートイメージをI2C経由で更新する場合

先に作った rtcl_p3s7_mipi.bin を SPI-ROM の 0x100000 から書き込むことで、アップデートを更新できます。

別途 rtcl_p3s7_flash_rom コマンドをビルドして、KV260 上で

```bash
rtcl_p3s7_flash_rom -v -w rtcl_p3s7_mipi.bin
```

と実行することで、アップデートイメージを書き換えて、ベリファイするようにしております。

#### ゴールデンイメージの作り方

こちらは JTAGダウンロードケーブルが必須ですが、

`projects/rtcl_p3s7/rtcl_p3s7_mipi/syn/tcl/` にて make することで、`rtcl_p3s7_mipi_golden_full.mcs` を作成できます。

このイメージは、ゴールデンイメージ、Timer1、アップデートイメージ、Timer2 の４つの領域を統合しております。
詳しくは XAPP1247 のドキュメントを参照ください。


## シミュレーション

projects/kv260/kv260_blinking_led/sim 以下にシミュレーション環境を作っています。

- projects/kv260/kv260_blinking_led/sim/xsim : xsim用

該当ディレクトリに移動して make と実行することで、シミュレーションが動きます。

.vcd ファイルとして波形が生成されるので、gtkwave などの波形ビューワーで確認ください。


## 各種設定

FPGAの内部動作や、イメージセンサーのSPIでアクセスするレジスタはI2Cから制御できるようにしております。

### FPGA設定

I2C経由で 16bitアドレス 16bit データの読み書きが可能で、以下のレジスタが操作できます。


|   Addr | 名称                 | Access | リセット値    | Bits/説明                                      |
|--------|----------------------|--------|--------------|-----------------------------------------------|
| 0x0000 | MODULE_ID            | RO     | 0x527A       | [15:0]=識別子                                  |
| 0x0001 | MODULE_VERSION       | RO     | 0x0108       | [15:0]=バージョン                              |
| 0x0002 | MODULE_CONFIG        | RO     | 0x0000       | [15:0]=モジュール設定値                        |
| 0x0003 | SW_RESET             | R/W    | 0x0001       | [0]=1を書き込むとソフトウェアリセット開始       |
| 0x0004 | SENSOR_ENABLE        | R/W    | 0x0000       | [0]=1: センサー有効                            |
| 0x0008 | SENSOR_READY         | RO     | -            | [0]=1: センサー初期化完了                      |
| 0x000c | SENSOR_PGOOD         | RO     | -            | [0]=1: センサー電源良好                        |
| 0x000d | SENSOR_PGOOD_EN      | R/W    | 0x0001       | [0]=1: PGOOD監視有効                           |
| 0x0010 | RECEIVER_RESET       | R/W    | 0x0001       | [0]=1: 受信系リセット                          |
| 0x0012 | RECEIVER_CLK_DLY     | R/W    | 0x0008       | [4:0]: 受信クロック遅延設定                    |
| 0x0020 | ALIGN_RESET          | R/W    | 0x0001       | [0]=1: アライメント部リセット                  |
| 0x0022 | ALIGN_PATTERN        | R/W    | 0x03A6       | [9:0]: パターン値                              |
| 0x0028 | ALIGN_STATUS         | RO     | -            | [1]=エラー, [0]=完了                           |
| 0x0040 | CLIP_ENABLE          | R/W    | 0x0001       | [0]=1: 画素値0を1にクリップ                    |
| 0x0050 | CSI_MODE             | R/W    | 0x0000       | [0]=0: 高速モード, [0]=1: CSI2モード           |
| 0x0052 | CSI_DT               | R/W    | 0x002B       | [7:0]: CSI2 Data Type                          |
| 0x0053 | CSI_WC               | R/W    | 0x0140       | [15:0]: CSI2 Word Count                        |
| 0x0080 | DPHY_CORE_RESET      | R/W    | 0x0001       | [0]=1: D-PHY コアリセット                      |
| 0x0081 | DPHY_SYS_RESET       | R/W    | 0x0001       | [0]=1: D-PHY SYSリセット                       |
| 0x0088 | DPHY_INIT_DONE       | RO     | -            | [0]=1: D-PHY 初期化完了                        |
| 0x00a0 | MMCM_CONTROL         | R/W    | 0x0000       | [0]=1: MMCMリセット, [1]=1: MMCMパワーダウン   |
| 0x00a1 | PLL_CONTROL          | R/W    | 0x0000       | [0]=1: PLLリセット, [1]=1: PLLパワーダウン     |
| 0x00b0 | PMOD_MODE            | R/W    | 0x0000       | [15:0]: PMOD機能モード設定                     |
| 0x00b2 | PMOD_GPIO_IN         | RO     | -            | [7:0]: PMOD GPIO入力値                         |
| 0x00b3 | PMOD_GPIO_OUT        | R/W    | 0x0000       | [7:0]: PMOD GPIO出力値                         |
| 0x00b4 | PMOD_GPIO_DIR        | R/W    | 0x0000       | [7:0]: PMOD GPIO方向 (1:出力, 0:入力)          |
| 0x00b8 | PMOD_TRG_SEL         | R/W    | 0x0000       | [1:0]: トリガ入力選択                          |
| 0x00b9 | PMOD_HDR_SEL         | R/W    | 0x0000       | [2:0]: HDRモード選択                           |
| 0x00bc | PMOD_SLOT_LEN        | R/W    | 0x0007       | PMOD パターン出力のスロット長 - 1    |
| 0x0200 | PMOD_SLOT0_PAT       | R/W    | 0x01         | タイムスロット0 での PMOD出力パターン |
| 0x0201 | PMOD_SLOT1_PAT       | R/W    | 0x02         | タイムスロット1 での PMOD出力パターン |
| 0x0202 | PMOD_SLOT2_PAT       | R/W    | 0x04         | タイムスロット2 での PMOD出力パターン |
| 0x0203 | PMOD_SLOT3_PAT       | R/W    | 0x08         | タイムスロット3 での PMOD出力パターン |
| 0x0204 | PMOD_SLOT4_PAT       | R/W    | 0x10         | タイムスロット4 での PMOD出力パターン |
| 0x0205 | PMOD_SLOT5_PAT       | R/W    | 0x20         | タイムスロット5 での PMOD出力パターン |
| 0x0206 | PMOD_SLOT6_PAT       | R/W    | 0x40         | タイムスロット6 での PMOD出力パターン |
| 0x0207 | PMOD_SLOT7_PAT       | R/W    | 0x80         | タイムスロット7 での PMOD出力パターン |
| 0x0208 | PMOD_SLOT8_PAT       | R/W    | 0x00         | タイムスロット8 での PMOD出力パターン |
| 0x0209 | PMOD_SLOT9_PAT       | R/W    | 0x00         | タイムスロット9 での PMOD出力パターン |
| 0x020a | PMOD_SLOT10_PAT      | R/W    | 0x00         | タイムスロット10 での PMOD出力パターン |
| 0x020b | PMOD_SLOT11_PAT      | R/W    | 0x00         | タイムスロット11 での PMOD出力パターン |
| 0x020c | PMOD_SLOT12_PAT      | R/W    | 0x00         | タイムスロット12 での PMOD出力パターン |
| 0x020d | PMOD_SLOT13_PAT      | R/W    | 0x00         | タイムスロット13 での PMOD出力パターン |
| 0x020e | PMOD_SLOT14_PAT      | R/W    | 0x00         | タイムスロット14 での PMOD出力パターン |
| 0x020f | PMOD_SLOT15_PAT      | R/W    | 0x00         | タイムスロット15 での PMOD出力パターン |
| 0x0300 | PMOD_SLOT0_TIM       | R/W    | 0xffff       | タイムスロット0 でのパターン出力時間 |
| 0x0301 | PMOD_SLOT1_TIM       | R/W    | 0xffff       | タイムスロット1 でのパターン出力時間 |
| 0x0302 | PMOD_SLOT2_TIM       | R/W    | 0xffff       | タイムスロット2 でのパターン出力時間 |
| 0x0303 | PMOD_SLOT3_TIM       | R/W    | 0xffff       | タイムスロット3 でのパターン出力時間 |
| 0x0304 | PMOD_SLOT4_TIM       | R/W    | 0xffff       | タイムスロット4 でのパターン出力時間 |
| 0x0305 | PMOD_SLOT5_TIM       | R/W    | 0xffff       | タイムスロット5 でのパターン出力時間 |
| 0x0306 | PMOD_SLOT6_TIM       | R/W    | 0xffff       | タイムスロット6 でのパターン出力時間 |
| 0x0307 | PMOD_SLOT7_TIM       | R/W    | 0xffff       | タイムスロット7 でのパターン出力時間 |
| 0x0308 | PMOD_SLOT8_TIM       | R/W    | 0xffff       | タイムスロット8 でのパターン出力時間 |
| 0x0309 | PMOD_SLOT9_TIM       | R/W    | 0xffff       | タイムスロット9 でのパターン出力時間 |
| 0x030a | PMOD_SLOT10_TIM      | R/W    | 0xffff       | タイムスロット10 でのパターン出力時間 |
| 0x030b | PMOD_SLOT11_TIM      | R/W    | 0xffff       | タイムスロット11 でのパターン出力時間 |
| 0x030c | PMOD_SLOT12_TIM      | R/W    | 0xffff       | タイムスロット12 でのパターン出力時間 |
| 0x030d | PMOD_SLOT13_TIM      | R/W    | 0xffff       | タイムスロット13 でのパターン出力時間 |
| 0x030e | PMOD_SLOT14_TIM      | R/W    | 0xffff       | タイムスロット14 でのパターン出力時間 |
| 0x030f | PMOD_SLOT15_TIM      | R/W    | 0xffff       | タイムスロット15 でのパターン出力時間 |


なお 0x1000 以降は MMCM の DRP ポートにレジスタにマップしており、なお 0x4000 以降は Python300のSPIレジスタにマップしています。

詳しくはソースコードや KV260 側のサンプルコードを参照ください。

## ブロック図

設計のブロック図を以下に示します。

![rtcl_p3s7_mipi_block_diagram](doc/rtcl_p3s7_mipi_block_diagram.png)

mode select にて、独自高速モードと CSI2モードを切り替えられます。

また、I2CでFPGAの各種設定やイメージセンサーのレジスタ設定が可能です。

MMCMのDRPポートもI2CからアクセスしてDPHYの速度を変更できる為、KV260接続時 1250Mbps、ZYBO Z7 接続時 950Mbps といったように、接続先に応じた速度設定が可能です。

