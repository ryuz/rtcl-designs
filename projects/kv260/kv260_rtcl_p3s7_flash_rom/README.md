# KV260 用 RTCL-P3S7-MIPI グローバルシャッター高速度カメラ Flash-ROM 更新アプリ

## 概要

KV260 で [グローバルシャッターMIPI高速度カメラ](https://rtc-lab.com/products/rtcl-cam-p3s7-mipi/)(設計は[こちら](https://github.com/ryuz/rtcl-p3s7-mipi-pcb)) の Flash-ROM を JTAG ダウンロードケーブルなしで更新するアプリです。

本READMEは、kv260_rtcl_p3s7_hs などのアプリが動作する準備が整っており、 KV260 側で Linux が起動済みで、`jelly-fpga-server` に接続できる前提で説明します。
...
すでに KV260 側で `jelly-fpga-server` が起動しており、SSH 接続できる場合は以下が最短です。

```bash
cd projects/kv260/kv260_rtcl_p3s7_flash_rom/app/rust
...
- Rust アプリのクロスビルド (aarch64)
- FPGA アクセラレータのロード
- 更新対象 bitstream (`rtcl_p3s7_mipi.bin`) の生成と転送
- KV260 上で Flash-ROM への書き込みと検証 (`-w -v`)
...
```bash
cd projects/kv260/kv260_rtcl_p3s7_flash_rom/syn/tcl
make
make bit_cp
```
...
`projects/kv260/kv260_rtcl_p3s7_flash_rom/app/rust/Makefile` の主なターゲットです。

- `make build` : ローカルビルド
- `make cross_build` : aarch64 向けクロスビルド
- `make remote_run` : KV260 へ転送して実行
