...
```bash
make run_rust RUN_OPT="--color --width 320 --height 320 --fps 1000"
```

などとすると、解像度やフレームレートを指定して実行できます。
...
```bash
make remote_run RUN_OPT="--color --width 320 --height 320 --fps 1000 --pgood-off"
```
...
RTCL-P3S7-MIPI グローバルシャッター高速度カメラは KV260 の 3.3V 電源のスペックに対してぎりぎりに近い電力を消費します。

ボード個体差によって若干の電圧降下が起こり、電圧監視している PGOOD 信号が立たずに起動しない場合があるようです。

その場合は、カメラモジュールのレジスタ 0x0d を 0x00 に設定すると、電圧監視を無効化でき、起動する場合があります。

なお、電圧監視機能の有無にかかわらず、動作中の電源切断は非推奨ですので、カメラアプリをクローズしてから KV260 の Linux をシャットダウン後に電源を切るようにしてください。
