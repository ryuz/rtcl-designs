...
impl RtclFifo32D3xx {
    pub fn new(dev_index: usize) -> Result<Self, Box<dyn Error>> {
        ...
        let (mut usb_tx, mut usb_rx) = D3xxDevice::new(dev_index)?;
...
//          axi4s_buf: Vec::<u8>::new(),
    ...
    fn recv_thread(device_rx: Arc<Mutex<Device>>, tx_command: mpsc::Sender<RtclPacket>, tx_stream: mpsc::Sender<RtclPacket>, rx_stop: mpsc::Receiver<()>) {
